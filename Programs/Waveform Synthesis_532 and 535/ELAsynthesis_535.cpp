...
/*	ELA_synthesis_535: Variable Timbre Generation	 */
/*	This program was developed and written independently in 
	Feb 2007 by
	Edwin L. Althouse, elalthouse@comcast.net.	*/

/*	Modifiec 1/2/07 to modify amplitudes of specified partials as a function 
	of frequency	*/
...
void Read_partial_indicies_real(char *fin);
void Read_spec_index(char *fin);
void Read_amp_mod(char *fin);
...
float t[5000],ff,amod;
float tmod[5000], shift[103], shft_ref;
float y[5000], yy[67000],AA[103],BB[103];
float output[67000];
float freq[55],timb_scale, dB_scale,spec_scale;
float temp2,temp3;
float time_offset_end, offset, offset_m,f_offset[103],partial_index[103],amp_mod[103];
...
void Read_myconfig532(char *fin)
{
	...
	exit(1); }
		...
		fscanf(fptr1,"%f",&spec_scale); //parameter for scaling transient partial amplitudes with frequency
										//try range of 0.5 to 0.7
...
void Read_spec_index(char *fin)
{
	...
	fptr0 = fopen(fin,"r");
	if (fptr0 == NULL) {
	printf("Cannot open spec_index.txt for reading.\n");
	exit(0); }
	else
	{
		for (i=0;i<=num_partials-1;i++) fscanf(fptr0,"%d", &spec_index[i]);
		fclose(fptr0);

		printf("Some spec_indicies are:\n");
		 for (i=0;i<=3;i++) printf("spec_index [%d]= %d\n",i,&spec_index[i]);
	}
}
/*	-------------------------------------------------------------------	*/
void Read_amp_mod(char *fin)
{
	...
	fptr0 = fopen(fin,"r");
	if (fptr0 == NULL) {
	printf("Cannot open amp_mod.txt for reading.\n");
	exit(0); }
	else
	{
		//	Read in values in dB and convert to decimal number
		for (i=0;i<=num_partials-1;i++) 
		{
			fscanf(fptr0,"%f", &amp_mod_dB);
			amp_mod[i]=pow(10.0,amp_mod_dB/20.0);
		}
		fclose(fptr0);

		printf("Some values of amp_mod are:\n");
		 for (i=0;i<=3;i++) printf("amp_mod [%d]= %E\n",i,&amp_mod[i]);
	}
}
/*	-------------------------------------------------------------------	*/
...
{ 
	...
	//	single out special transient partials and scale amplitude with freq
	if(spec_index[kk-1]==1) amod=pow(10.0,spec_scale*log10(freq0/freq));
	else amod=1.0;
	//	tweak final amplitude of any partial
		amod=amod*amp_mod[kk-1];
	...
	{		if (index*freq < 18000.) gg=1.0;
		...
		phase_offset=BB[kk-1];
			...
			phase_offset=phase_offset*degtorad ;
			output[jj]=output[jj] + timb_scale*gg*amod*yy[jj]*sin(freq*index*w*jj+phase_offset);
...
/*	Now normalize the maximum amplitude of the output[] array to 3E4.	*/
	...
	printf("vmax = %e\n",vmax);
	for (n=0;n<=maxpts-1;n++) output[n] = 28000.0f*output[n]/vmax;
...
/*	Read in logical (1,0; 1=yes, 0=no) to determine if amplitude of this partial
	should be scaled with frequency	*/
fin2=filename8;
Read_spec_index(fin2);
